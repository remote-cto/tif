  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const collegeRes = await fetch(`/api/college-info?college_id=${adminInfo.college_id}`);
        if (collegeRes.ok) {
        ...
        for (const student of studentsWithScores) {
          try {
            ...
            console.error(`Error fetching topic scores for student ${student.id}:`, err);
          }
        }
        ...
        <div className="bg-white shadow-md rounded-lg p-6 mb-8">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-center md:text-left">
              ...
              <div>
                <p className="text-sm text-blue-600 font-medium">College Name</p>
                <p className="text-lg font-semibold text-slate-800">
                          ...
                          <div key={assessment.id} className="bg-gray-50 rounded-lg p-4">
                            ...
                            <div className="space-y-1">
                              <div className="flex justify-between">
                                <span className="text-sm text-gray-600">Basic Score:</span>
                                <span className={`text-sm font-semibold ${getScoreColor(assessment.score_percent)}`}>
                                  {assessment.score}/{assessment.total_questions} ({assessment.score_percent?.toFixed(1)}%)
                                </span>
                              </div>
                              {assessment.total_score !== undefined && (
                                <div className="flex justify-between">
                                  <span className="text-sm text-gray-600">Total Score:</span>
                                  <span className={`text-sm font-semibold ${getScoreColor(assessment.total_score)}`}>
                              ...
                              {assessment.readiness_score !== undefined && (
                                <div className="flex justify-between">
                                  ...
                                  <span className={`text-sm font-semibold ${getScoreColor(assessment.readiness_score)}`}>
                    ...
                    <div className="p-6 bg-gray-50">
                      ...
                      {student.topicScores && student.topicScores.length > 0 ? (
                          ...
                          {student.topicScores.map((topic) => (
                            ...
                            >
                              <div className="flex justify-between items-start mb-2">
                                ...
                                <span className={`text-xs px-2 py-1 rounded-full border ${getClassificationColor(topic.classification)}`}>
                              ...
                              <div className="space-y-1 text-sm">
                                ...
                                <div className="flex justify-between">
                                  ...
                                  <span className={`font-medium ${getScoreColor(Number(topic.weighted_score))}`}>
                                    {Number(topic.weighted_score || 0).toFixed(1)}
                                  </span>
                                </div>
                                <div className="flex justify-between">
                                  <span className="text-gray-600">Normalized:</span>
                                  <span className={`font-medium ${getScoreColor(Number(topic.normalized_score))}`}>
                                ...
                                <div className="w-full bg-gray-200 rounded-full h-2">
                                  <div
                                    className={`h-2 rounded-full transition-all duration-300 ${
                                      Number(topic.normalized_score) >= 80 ? 'bg-green-500' :
                                      Number(topic.normalized_score) >= 60 ? 'bg-yellow-500' :
                                      'bg-red-500'
                                    }`}
                                    style={{ width: `${Math.min(Number(topic.normalized_score || 0), 100)}%` }}
                                  ></div>
                                </div>
                              </div>
                            </div>
                        ...
                        <div className="text-center py-8">
                          <p className="text-gray-500">No topic-wise scores available for this student.</p>
                        </div>
                      )}
                    </div>
                  )}
